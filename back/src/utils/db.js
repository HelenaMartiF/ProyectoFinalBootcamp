...
const connect = async () => {
    try {
        ...
        const {name, host} = db.connection /* guardamos name y host que viene de la conexión de la url */

        console.log(`Conectado a la base de datos ${name} DB en el host ${host}`) /* nos devuelve la info de quien está conectado y donde */
    } catch (error) {
