...
const usedEmail = async(email) =>{ /* esta función es async porque tiene que hacer una petición independiente. */
    const users = await User.find({email:email}); /* en nuestra colección vas a buscar dentro del campo email si existe el email que te estoy pasando  */
