...
try{
  const allListas = await Lista.find().populate("pelicula")/* el find lo trae todo */
...
const postListas = async (req, res)=>{
  ...
  const putListas = async (req, res) => {
    ...
    try {
      ...
      const updatedListas = await Listas.findByIdAndUpdate(
        id,
        putListas
  ...
  const deleteListas = async (req, res) => {
    try {
      ...
      const deletedListas = await Listas.findByIdAndDelete(
        id
