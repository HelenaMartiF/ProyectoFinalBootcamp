...
const register = async (req, res) => { /* Crear usuario */
    try {
...
const login = async (req, res) => {//6.generamos un token para el login generamos la funcion asincrona y lo importamos en users.routes
    try {
      ...
      if (!bcrypt.compareSync(req.body.password, userInfo.password)) {//comprobamos que las contraseñas sean iguales y compara lo que viene del body con el user info
